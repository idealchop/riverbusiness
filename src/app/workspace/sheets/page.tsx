...
import { Grid, Clock, History, ArrowUpRight } from 'lucide-react';
...
export default function SheetsHubPage() {
  ...
  // Fetch only 'sheet' types SCOPED by companyId
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        <div className="space-y-1">
            <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900">Recent Sheets</h1>
                    <p className="text-sm font-medium text-slate-500">Access your operational grids and data tables.</p>
                </div>
            </div>
        </div>
        ...
        <div className="grid gap-3">
            {isLoading ? (
                    ...
                    <Card key={page.id} className="border border-slate-100 shadow-none group hover:border-primary/20 transition-all duration-300 rounded-2xl bg-white">
                        <CardContent className="p-5 flex items-center justify-between">
                            <div className="flex items-center gap-4 min-w-0">
                                <div className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-green-50 group-hover:text-green-600 transition-colors shrink-0">
                                    <Grid className="h-5 w-5" />
                                </div>
                                <div className="min-w-0">
                                    <h3 className="text-base font-bold text-slate-900 truncate">{page.title || 'Untitled sheet'}</h3>
                ...
                <div className="py-24 text-center opacity-30 flex flex-col items-center gap-4 border-2 border-dashed rounded-3xl">
                    ...
                    <p className="text-sm font-bold uppercase tracking-widest text-slate-400">No sheets found</p>
                </div>
            )}
        </div>
      </div>
