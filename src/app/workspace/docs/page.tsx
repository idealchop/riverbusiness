...
import { FileText, Clock, Plus, Search, UserCircle, FolderPlus, ChevronDown, MoreHorizontal, Globe, Lock } from 'lucide-react';
        ...
        <div className="px-8 py-6 space-y-6 shrink-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-2">
                <div className="relative w-full md:w-96 group">
                    ...
                    <Input 
                        placeholder="Search for documents, folders, and tags..." 
                        value={searchTerm}
                ...
                <div className="flex items-center gap-2">
                    <Button variant="ghost" size="sm" className="h-9 px-3 gap-2 font-bold text-[11px] text-slate-500 uppercase tracking-widest hover:bg-slate-50">
                        Upload date <ChevronDown className="h-3 w-3" />
                    </Button>
                    <Button variant="ghost" size="sm" className="h-9 px-3 gap-2 font-bold text-[11px] text-slate-500 uppercase tracking-widest hover:bg-slate-50">
            ...
            <div className="p-8 pb-32">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                    {isLoading ? (
                        ...
                        <AssetCard key={page.id} page={page} type="doc" />
                    ))}
                    {!isLoading && filteredDocs.length === 0 && (
                        <div className="col-span-full py-40 text-center flex flex-col items-center gap-6 opacity-30 grayscale">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Create a new document to start your workspace</p>
                            </div>
                        </div>
                    )}
                </div>
...
function AssetCard({ page, type }: { page: CollabPage, type: 'doc' | 'sheet' | 'board' }) {
    const firestore = useFirestore();
    ...
    return (
        <Link href={`/workspace/${page.id}`} className="group block">
            <Card className="border-none shadow-none bg-white rounded-2xl overflow-hidden transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-slate-200">
                <div className="relative aspect-[1.6/1] w-full bg-slate-50 overflow-hidden border border-slate-100 rounded-2xl transition-all group-hover:border-primary/20">
                    <Image 
                    ...
                    <div className="absolute top-3 left-3 z-10">
                        <div className="h-8 w-8 rounded-xl bg-white/90 backdrop-blur-md shadow-sm border border-white/20 flex items-center justify-center">
                            {page.icon ? <span className="text-sm">{page.icon}</span> : <FileText className="h-4 w-4 text-blue-500" />}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-4 pt-3 border-t border-slate-50">
                        ...
                        {page.isPublic && (
                             <div className="flex items-center gap-1.5 text-[9px] font-black uppercase text-primary">
                                <Globe className="h-2.5 w-2.5" /> Public link
                            </div>
                        )}
                    </div>
                </CardContent>
