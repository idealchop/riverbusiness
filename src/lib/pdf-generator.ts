...
export const generateMonthlySOA = async ({ user, deliveries, sanitationVisits, complianceReports, billingPeriod, branches, transactions }: MonthlySOAProps) => {
    ...
    if (isParent && transactions) {
        ...
        const summaryBody = [['Total Credits (Top-Ups)', `P ${totalCredits.toLocaleString()}`], ['Total Debits (Branch Consumption)', `P ${totalDebits.toLocaleString()}`], ['Final Balance', `P ${finalBalance.toLocaleString()}`]];
    ...
    if (deliveries.length > 0) {
        ...
        const deliveryBody = deliveries.map(d => {
            const qty = d.volumeContainers || 0; const liters = d.liters ?? containerToLiter(qty); const deliveryAmount = d.amount ?? (liters * pricePerLiter);
            totalQty += qty; totalLiters += liters; totalAmount += deliveryAmount;
            return [d.id, format(new Date(d.date), 'MMM d, yyyy'), ...(isParent ? [branchMap[d.userId] || d.userId] : []), qty, `P ${pricePerContainer.toFixed(2)}`, `${liters.toFixed(1)} L`, `P ${deliveryAmount.toFixed(2)}`, ...(isParent ? [] : [d.status])];
        });
        const summaryRow = [{ content: 'TOTAL CONSUMPTION', colSpan: isParent ? 3 : 2, styles: { fontStyle: 'bold', halign: 'right' } }, { content: totalQty.toString(), styles: { fontStyle: 'bold' } }, { content: '' }, { content: `${totalLiters.toFixed(1)} L`, styles: { fontStyle: 'bold' } }, { content: `P ${totalAmount.toLocaleString(undefined, {minimumFractionDigits: 2})}`, styles: { fontStyle: 'bold' } }, ...(isParent ? [] : [{ content: '' }])];
...
export const generateInvoicePDF = async ({ user, invoice }: InvoicePDFProps) => {
    ...
    doc.setFillColor(83, 142, 194); doc.rect(0, 0, pageWidth, 100, 'F');
    if (logoBase64) doc.addImage(logoBase64, 'PNG', margin, 30, 40, 40);
    doc.setFontSize(22); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255); doc.text('River Tech Inc.', margin + 50, 55);
    doc.setFontSize(12); doc.text('Invoice Receipt', margin + 50, 75);
    let lastY = 140; const invoiceDate = typeof invoice.date === 'string' ? new Date(invoice.date) : (invoice.date as any).toDate();
    doc.setTextColor(0); doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.text('Invoice #:', margin, lastY); doc.setFont('helvetica', 'normal'); doc.text(invoice.id, margin + 80, lastY);
    doc.setFont('helvetica', 'bold'); doc.text('Date:', margin, lastY + 15); doc.setFont('helvetica', 'normal'); doc.text(format(invoiceDate, 'MMMM d, yyyy'), margin + 80, lastY + 15);
    lastY += 50; doc.setFont('helvetica', 'bold'); doc.setTextColor(83, 142, 194); doc.text('BILL TO:', margin, lastY); doc.setFont('helvetica', 'normal'); doc.setTextColor(0);
    doc.text(user.businessName || '', margin, lastY + 15); doc.text(user.address || '', margin, lastY + 27, { maxWidth: pageWidth / 2 });
    doc.text(`Client ID: ${user.clientId || 'N/A'}`, margin, lastY + 39); doc.text(user.email || '', margin, lastY + 51);
    lastY += 90;
    autoTable(doc, {
        startY: lastY,
    ...
    const summaryX = pageWidth - margin - 200; const vatIncluded = invoice.amount * (12/112); lastY = (doc as any).lastAutoTable.finalY + 30;
    const totals = [['Subtotal (VAT Included)', `P ${invoice.amount.toLocaleString(undefined, {minimumFractionDigits: 2})}`], ['VAT (12% Included)', `P ${vatIncluded.toLocaleString(undefined, {minimumFractionDigits: 2})}`], ['Total Paid', `P ${invoice.amount.toLocaleString(undefined, {minimumFractionDigits: 2})}`]];
    doc.setFontSize(10);
    totals.forEach((total, index) => {
        doc.setFont('helvetica', index === totals.length - 1 ? 'bold' : 'normal');
        doc.text(total[0], summaryX, lastY + (index * 18)); doc.text(total[1], pageWidth - margin, lastY + (index * 18), { align: 'right'});
    });
