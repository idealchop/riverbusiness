...
export function MyAccountDialog({ user, authUser, planImage, paymentHistory, paymentsLoading, onLogout, children, onPayNow, isOpen, onOpenChange, initialTab }: MyAccountDialogProps) {
  ...
  const handleProfilePhotoUpload = async () => {
    ...
    startTransition(() => {
        ...
        .then((downloadURL) => {
            const userDocRef = doc(firestore, 'users', authUser.uid);
            return updateDoc(userDocRef, { photoURL: downloadURL });
        })
        .then(() => {
            toast({ title: 'Profile Photo Updated!', description: 'Your new photo has been saved.' });
        })
        .catch((error) => {
            console.error("Profile photo upload/update failed:", error);
            toast({ variant: "destructive", title: "Update Failed", description: "Could not update your profile photo." });
        })
        .finally(() => {
