...
export function SheetEditor({ initialData, onContentChange, editable = true }: SheetEditorProps) {
  ...
  const [records, setRecords] = useState<SheetRecord[]>(initialData?.records || [
      { id: 'r1', values: { f1: 'Initialize Workspace', f2: 'In Progress', f3: '2025-06-01' }, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
  ]);
  ...
  const addField = (type: SheetFieldType) => {
    ...
    toast({ title: 'Column initialized', description: `Added high-fidelity ${type} field.` });
  };

  const deleteField = (fieldId: string) => {
    const fieldToDelete = fields.find(f => f.id === fieldId);
    if (fieldToDelete?.isPrimary) {
        toast({ variant: 'destructive', title: 'Action blocked', description: 'The primary identifier column cannot be deleted.' });
        return;
    }
        ...
        <div className="h-14 border-b flex items-center justify-between px-4 sm:px-6 bg-white shrink-0 z-30">
            <div className="flex items-center gap-3">
                ...
                <div className="hidden sm:flex items-center gap-1.5">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => toast({ title: 'Logic builder', description: 'Filter rules are coming soon.' })} className="h-8 px-3 rounded-lg gap-2 text-slate-500 hover:text-slate-900 font-bold text-[10px] uppercase tracking-wider">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent className="w-48 rounded-xl p-1 shadow-2xl border-slate-100">
                                            <DropdownMenuItem className="gap-2 text-xs font-semibold rounded-lg cursor-pointer" onClick={() => {
                                                const name = prompt('Enter new column identifier:', field.name);
                                                if (name) renameField(field.id, name);
                                            }}>
                                                <Edit className="h-3.5 w-3.5" /> Rename Field
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="gap-2 text-xs font-semibold rounded-lg cursor-pointer" onClick={() => toast({ title: 'Protocol Lock', description: 'Schema type conversion is restricted to maintain data integrity.' })}>
            ...
            <div className="absolute inset-y-0 right-0 w-full sm:w-[500px] bg-white border-l shadow-3xl z-[100] animate-in slide-in-from-right duration-300 flex flex-col">
                ...
                <ScrollArea className="flex-1">
                    <div className="p-10 space-y-10">
                        ...
                        <div className="space-y-6">
                            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Collaborative Metadata</h4>
                ...
                <div className="h-20 border-t p-6 bg-slate-50/50 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Button variant="ghost" className="rounded-xl h-9 text-[10px] font-bold uppercase tracking-widest text-slate-400" onClick={() => setSelectedRecordId(null)}>Dismiss</Button>
                        <Button onClick={() => { setSelectedRecordId(null); toast({ title: 'Record synchronized' }); }} className="rounded-xl h-9 px-6 text-[10px] font-bold uppercase tracking-widest shadow-lg">Finalize Update</Button>
                    </div>
                </div>
            </div>
        )}
